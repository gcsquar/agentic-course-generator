...
def segment(source: IngestResult, *, mock: bool = False, use_llm: bool = True,
    ...
    system_prompt = (
        ...
        "RULES:\n"
        "- Create ONE segment per distinct subtopic. The count SCALES with the article: a short "
        "piece may need 3-5 segments, a long one MANY more (a ~150-paragraph article usually "
        "needs 12-20). Never fewer than 3; returning a single segment is WRONG.\n"
        "- Do NOT cram several unrelated subtopics into one segment. A segment that covers a large "
        "stretch of the article almost always should be split into smaller standalone lessons.\n"

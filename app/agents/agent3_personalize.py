...
def _generate_draft(user: UserProfile, lesson: Lesson, curriculum: Curriculum,
    ...
    system = (
        ...
        "FAITHFULNESS — read this before anything else.\n"
        "Your ONLY source of facts is the ORIGINAL LESSON text and the ALREADY TAUGHT list. "
        "Treat your own knowledge of the topic as non-existent: act as though you know only "
        "what the source says. These specific patterns are hallucinations, NOT personalization "
        "— any of them will be caught and the whole lesson sent back:\n"
        "• Adding numeric examples (matrices, equations, specific values, percentages) that are "
        "NOT in the source — even when they would be pedagogically useful.\n"
        "• Describing what a named tool, paper, method, or algorithm does/is for when the source "
        "only lists its name. Example: source says 'SVD is another approach' → write exactly "
        "that, do NOT add when SVD is used or what problem it solves.\n"
        "• Strengthening a stated relationship: 'key to understanding X' must NOT become "
        "'gives the exact X' or 'is equivalent to X'. Quote the source's actual claim.\n"
        "• Adding tradeoffs, critiques, or comparisons the source does not state.\n\n"
        ...
        "  Quantitative thinker (business, finance, engineering, data): WHEN the source contains "
        "numbers, metrics, or benchmarks, surface and lead with them. WHEN it does not, present "
        "qualitatively — never invent numeric examples (matrices, values, percentages, '~3x faster') "
        "to make the lesson feel more concrete; an invented number is hallucination regardless of "
        "how pedagogically natural it seems for this learner type.\n"
        "  Qualitative thinker (humanities, arts, general audience, creative roles): prefer "
        "descriptive language. Numbers from the source are fine; don't add or invent metrics.\n"
        "  When in doubt, look at the learner's role and interests: a manager/analyst wants "
        "numbers from the source surfaced; a student or general reader is fine with words.\n\n"
        ...
        "- If the source only NAMES or LISTS something without describing it (a linked article, "
        "reference, tool, or algorithm), give only what the source says about it. If the source "
        "says 'SVD is another approach', write 'SVD is another approach' — do NOT add when to "
        "use it, what problem it solves, or how it compares. Your training knowledge about that "
        "item is irrelevant; you know only what this source says.\n"
